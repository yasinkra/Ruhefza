    ...
    return (
        <AppShell>
            <div className="max-w-3xl mx-auto py-8 px-4">
                ...
                <div className="glass-effect rounded-2xl shadow-xl border border-white/50 overflow-hidden mb-8 transition-transform duration-300 hover:shadow-2xl">
                    {/* Cover & Avatar Area */}
                    <div className="h-40 bg-gradient-to-r from-violet-600 via-indigo-600 to-sky-500 relative">
                        ...
                        <div className="absolute -bottom-14 left-8">
                            <Avatar className="h-28 w-28 border-4 border-white shadow-xl bg-white ring-2 ring-slate-100">
                                ...
                                <AvatarFallback className="text-4xl bg-gradient-to-br from-slate-100 to-slate-200 text-slate-700">
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div>
                                    <h2 className="text-3xl font-bold text-slate-900 flex items-center gap-3 tracking-tight">
                                        {profile.full_name}
                                        <span className="px-3 py-1 rounded-full bg-slate-100 text-slate-600 text-xs font-semibold uppercase tracking-wider border border-slate-200">
                                    ...
                                    <div className="flex items-center gap-3 mt-3">
                                        <div className="inline-flex items-center gap-1.5 bg-slate-900 text-slate-50 px-3 py-1.5 rounded-lg text-xs font-mono font-medium shadow-md transition-transform hover:scale-105 active:scale-95 cursor-pointer group"
                                            onClick={() => {
                                        ...
                                        >
                                            ...
                                            <Copy className="h-3 w-3 text-slate-400 group-hover:text-white transition-colors" />
                                        </div>
                                        <span className="text-xs text-slate-500 font-medium">Bu ID size özeldir.</span>
                                    </div>
                                </div>
                            </div>

                            {/* Username Section */}
                            <div className="mt-8 p-1 bg-gradient-to-r from-slate-100 to-slate-50 rounded-2xl border border-slate-200/60 shadow-inner">
                                <div className="bg-white/60 p-5 rounded-xl backdrop-blur-sm">
                                    <Label className="text-xs text-slate-500 font-bold uppercase tracking-widest mb-3 block flex items-center gap-1">
                                        <span className="w-1 h-1 bg-sky-500 rounded-full inline-block"></span> Kullanıcı Adı
                                    </Label>

                                    {isEditingUsername ? (
                                        <div className="flex items-center gap-2 max-w-sm">
                                            <div className="relative flex-1 group">
                                                <span className="absolute left-3 top-2.5 text-slate-400 group-focus-within:text-sky-500 transition-colors font-medium">@</span>
                                                <Input
                                                    value={usernameInput}
                                                    onChange={(e) => setUsernameInput(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))}
                                                    className="pl-7 bg-white border-slate-200 focus-visible:ring-sky-500 font-medium"
                                                    placeholder="kullanici_adi"
                                                    maxLength={20}
                                                />
                                            </div>
                                            <Button size="sm" onClick={handleUsernameUpdate} disabled={usernameLoading} className="bg-sky-600 hover:bg-sky-700 text-white shadow-md shadow-sky-200">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="text-lg text-slate-700 font-semibold tracking-tight">
                                                {profile.username ? <span className="text-sky-700">@{profile.username}</span> : <span className="text-slate-400 font-normal italic">Kullanıcı adı oluşturulmadı</span>}
                                            </div>
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                className="border-slate-200 text-slate-600 hover:text-sky-600 hover:border-sky-200 hover:bg-sky-50 transition-all shadow-sm"
                                                onClick={() => {
                        ...
                        {isEditing ? (
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="space-y-4 pt-2">
                                    <Label className="text-slate-700 font-bold flex items-center gap-2">
                                        <ShieldCheck className="h-4 w-4 text-sky-500" /> Profil Deseni Seçin
                                    </Label>
                                    <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
                                        {NEUTRAL_AVATARS.map((avatar, index) => (
                                            ...
                                            >
                                                ...
                                                {formData.avatar_url === avatar && (
                                                    <div className="absolute top-0 right-0 bg-sky-500 text-white p-0.5 rounded-bl-lg">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                                    <div className="space-y-2">
                                        <Label htmlFor="fullname" className="text-slate-700 font-bold">Ad Soyad</Label>
                                        <Input
                                            id="fullname"
                                            className="bg-slate-50 border-slate-200 focus:bg-white h-11 rounded-xl transition-all"
                                            value={formData.full_name || ""}
                                            onChange={(e) => setFormData({ ...formData, full_name: e.target.value })}
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="avatar_url" className="text-slate-700 font-bold">Özel URL (Opsiyonel)</Label>
                                        <Input
                                            id="avatar_url"
                                            className="bg-slate-50 border-slate-200 focus:bg-white h-11 rounded-xl transition-all"
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <Label htmlFor="bio" className="text-slate-700 font-bold flex items-center gap-2">
                                            <FileText className="h-4 w-4 text-sky-500" /> Biyografi
                                        </Label>
                                        <span className={cn(
                                            "text-[10px] font-bold px-2 py-0.5 rounded-full",
                                            (formData.bio?.length || 0) > 450 ? "bg-red-50 text-red-500" : "bg-slate-50 text-slate-400"
                                        )}>
                                    ...
                                    <Textarea
                                        id="bio"
                                        className="bg-slate-50 border-slate-200 focus:bg-white focus:ring-0 focus:border-sky-300 transition-all resize-none rounded-xl"
                                        value={formData.bio || ""}
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <Label htmlFor="special_note" className="text-slate-700 font-bold flex items-center gap-2">
                                            <Edit2 className="h-4 w-4 text-violet-500" />
                                            {profile.role === 'teacher' ? 'Uzmanlık Alanı / Detaylar' : 'Özel Notlar'}
                                        </Label>
                                        <span className={cn(
                                            "text-[10px] font-bold px-2 py-0.5 rounded-full",
                                            (formData.special_note?.length || 0) > 900 ? "bg-red-50 text-red-500" : "bg-slate-50 text-slate-400"
                                        )}>
                                    ...
                                    <Textarea
                                        id="special_note"
                                        className="bg-slate-50 border-slate-200 focus:bg-white focus:ring-0 focus:border-violet-200 transition-all resize-none rounded-xl"
                                ...
                                <div className="pt-6 border-t border-slate-100/80">
                                    <div className="flex items-center gap-2 mb-5">
                                        ...
                                        <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Bağlantılar</h3>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="group space-y-2">
                                            <Label className="flex items-center gap-2 text-xs font-bold text-slate-500 group-focus-within:text-pink-500 transition-colors">
                                                <Instagram className="h-4 w-4" /> Instagram
                                            </Label>
                                            <Input
                                                placeholder="@kullanici_adi"
                                                className="bg-slate-50 border-slate-200 focus:bg-white rounded-xl h-11"
                                                value={getSocialLinks(formData).instagram}
                                                onChange={(e) => updateSocialLink('instagram', e.target.value)}
                                            />
                                        </div>
                                        <div className="group space-y-2">
                                            <Label className="flex items-center gap-2 text-xs font-bold text-slate-500 group-focus-within:text-sky-500 transition-colors">
                                        ...
                                        <div className="group space-y-2">
                                            <Label className="flex items-center gap-2 text-xs font-bold text-slate-500 group-focus-within:text-blue-600 transition-colors">
                                        ...
                                        <div className="group space-y-2">
                                            <Label className="flex items-center gap-2 text-xs font-bold text-slate-500 group-focus-within:text-green-500 transition-colors">
                                        ...
                                        <div className="group space-y-2 md:col-span-2">
                                            <Label className="flex items-center gap-2 text-xs font-bold text-slate-500 group-focus-within:text-indigo-600 transition-colors">
                                ...
                                <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
                                    ...
                                    <Button onClick={handleSave} disabled={saving} className="bg-slate-900 hover:bg-slate-800 text-white shadow-xl shadow-slate-200 rounded-xl px-6 font-bold h-11">
                            ...
                            <div className="space-y-8 animate-in fade-in duration-500">
                                ...
                                <Tabs defaultValue="about" className="w-full">
                                    <TabsList className="grid w-full grid-cols-3 mb-10 h-14 bg-slate-100/50 p-1.5 rounded-2xl">
                                        <TabsTrigger value="about" className="rounded-xl data-[state=active]:bg-white data-[state=active]:shadow-md data-[state=active]:text-sky-600 transition-all font-black text-sm uppercase tracking-widest py-2.5">
                                            Hakkında
                                        </TabsTrigger>
                                        <TabsTrigger value="bookmarks" className="rounded-xl data-[state=active]:bg-white data-[state=active]:shadow-md data-[state=active]:text-sky-600 transition-all font-black text-sm uppercase tracking-widest py-2.5 flex items-center justify-center gap-2">
                                    ...
                                    <TabsContent value="about" className="space-y-8 mt-0 animate-in fade-in slide-in-from-bottom-3 duration-500">
                                        <div className="group relative">
                                            <div className="flex items-center gap-3 mb-5">
                                                <div className="w-1.5 h-6 bg-sky-500 rounded-full group-hover:h-8 transition-all duration-300"></div>
                                                <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">
                                                    Biyografi
                                                </h3>
                                            </div>
                                            <div className="bg-white p-7 rounded-[32px] border border-slate-100 shadow-sm leading-relaxed text-slate-600 group-hover:border-sky-100 group-hover:shadow-md transition-all duration-300 relative overflow-hidden">
                                                <div className="absolute top-0 right-0 w-32 h-32 bg-sky-50 rounded-full -mr-16 -mt-16 opacity-50 group-hover:scale-110 transition-transform duration-500"></div>
                                                {profile.bio ? (
                                                    <p className="whitespace-pre-wrap relative z-10 font-medium">{profile.bio}</p>
                                                ) : (
                                                    <p className="text-slate-400 italic font-medium relative z-10">Kendinizden henüz bahsetmediniz. Düzenleyerek başlayabilirsiniz.</p>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="group relative">
                                            ...
                                            <div className="bg-slate-50/70 p-7 rounded-[32px] border border-slate-100 shadow-inner group-hover:bg-slate-50 transition-all duration-300 relative overflow-hidden">
                                                ...
                                                {profile.special_note ? (
                                                    <p className="text-slate-700 whitespace-pre-wrap font-medium relative z-10">{profile.special_note}</p>
                                                ) : (
                                                    <p className="text-slate-400 italic relative z-10">Ek bilgi belirtilmemiş.</p>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        {profile.social_links && Object.values(profile.social_links).some(v => v) && (
                                            <div className="pt-8 border-t border-slate-100/80">
                                                <h3 className="text-xs font-black text-slate-400 mb-6 uppercase tracking-[0.2em] text-center">Bağlantıda Kalalım</h3>
                                                <div className="flex flex-wrap justify-center gap-4">
                                                    ...
                                                    {profile.social_links.twitter && (
                                                        <a href={profile.social_links.twitter.startsWith('http') ? profile.social_links.twitter : `https://twitter.com/${profile.social_links.twitter}`}
                                                            target="_blank" rel="noopener noreferrer"
                                                            className="flex items-center gap-3 px-6 py-3 bg-slate-900 text-white rounded-2xl shadow-lg shadow-slate-200 hover:shadow-slate-300 hover:-translate-y-1 transition-all">
                                                    ...
                                                    {profile.social_links.website && (
                                                        <a href={profile.social_links.website}
                                                            target="_blank" rel="noopener noreferrer"
                                                            className="flex items-center gap-3 px-6 py-3 bg-white text-slate-900 border border-slate-200 rounded-2xl shadow-sm hover:shadow-md hover:-translate-y-1 transition-all group">
                                                            <Globe className="h-5 w-5 text-sky-500" />
                                                            <span className="font-black text-xs uppercase tracking-wider group-hover:text-sky-600 transition-colors">Portfolyo</span>
                                    ...
                                    <TabsContent value="bookmarks" className="mt-0 animate-in fade-in slide-in-from-bottom-3 duration-500">
                                        {loadingBookmarks ? (
                                            <div className="py-20 flex flex-col items-center gap-4">
                                                ...
                                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Kayıtlar yükleniyor...</p>
                                            </div>
                                        ) : (bookmarkedPosts.length === 0 && bookmarkedArticles.length === 0) ? (
                                            <div className="text-center py-20 bg-white rounded-[40px] border border-dashed border-slate-200 px-6">
                                                <div className="mx-auto w-24 h-24 bg-slate-50 rounded-[32px] flex items-center justify-center mb-6 border border-slate-100 transition-transform duration-500 hover:scale-110">
                                                    <Bookmark className="h-10 w-10 text-slate-200" />
                                                </div>
                                                <h3 className="text-xl font-black text-slate-800 mb-2 tracking-tight">Kayıtlı İçerik Yok</h3>
                                                <p className="text-slate-400 font-medium text-sm max-w-[240px] mx-auto leading-relaxed">İlgini çeken paylaşım ve makaleleri buraya kaydedebilirsin.</p>
                                            </div>
                                        ) : (
                                                ...
                                                {bookmarkedArticles.length > 0 && (
                                                    <div>
                                                        <div className="flex items-center gap-2 mb-4">
                                                            ...
                                                            <h3 className="font-bold text-slate-700 text-sm uppercase tracking-widest">Makaleler</h3>
                                                        </div>
                                                        <div className="grid grid-cols-1 gap-4">
                                                            {bookmarkedArticles.map((art) => (
                                                                <div key={art.id} className="flex gap-3 p-4 bg-white rounded-2xl border border-slate-100 hover:border-sky-200 hover:shadow-md transition-all group items-start">
                                                                    <a href={`/knowledge/${art.id}`} className="flex-1 min-w-0 block">
                                                                        <span className="text-[10px] font-bold uppercase tracking-wider text-sky-600 bg-sky-50 px-2 py-0.5 rounded-full">{art.category}</span>
                                                                        <h4 className="font-bold text-slate-900 mt-2 mb-1 text-sm leading-snug group-hover:text-sky-700 transition-colors line-clamp-2">{art.title}</h4>
                                                                        <p className="text-xs text-slate-400 line-clamp-1">{art.author?.full_name}</p>
                                                                    </a>
                                                                    <button
                                                                        onClick={async () => {
                                                                        ...
                                                                        className="shrink-0 p-1.5 rounded-full text-slate-300 hover:text-red-400 hover:bg-red-50 transition-colors mt-0.5"
                                                ...
                                                {bookmarkedPosts.length > 0 && (
                                                    <div>
                                                        <div className="flex items-center gap-2 mb-4">
                                                            ...
                                                            <h3 className="font-bold text-slate-700 text-sm uppercase tracking-widest">Gönderiler</h3>
                                                        </div>
