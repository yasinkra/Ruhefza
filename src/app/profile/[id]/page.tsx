        ...
        <AppShell>
            <div className="max-w-4xl mx-auto px-4 py-8 pb-20">
                ...
                <button onClick={() => router.back()}
                    className="flex items-center gap-2 text-gray-400 hover:text-gray-800 mb-8 transition-all group">
                    <div className="p-2 rounded-full bg-gray-100 group-hover:bg-white group-hover:shadow-sm transition-all">
                ...
                {/* Profile Header Card */}
                <div className="bg-white rounded-[40px] shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-gray-100 overflow-hidden mb-8 group transition-all duration-500 hover:shadow-[0_30px_70px_rgba(0,0,0,0.06)]">
                    {/* Cover Area */}
                    <div className={cn(
                        "h-48 relative overflow-hidden transition-all duration-700",
                        profile.role === 'teacher' ? "bg-gradient-to-br from-[#0c9789] via-[#14b8a6] to-[#0a7c70]" :
                            profile.role === 'student' ? "bg-gradient-to-br from-[#0c9789] via-[#14b8a6] to-[#0a7c70]" :
                        ...
                        <div className="absolute -bottom-16 left-10 z-10">
                            <div className="relative group/avatar">
                                ...
                                <Avatar className="h-32 w-32 border-[6px] border-white shadow-2xl rounded-[32px] bg-white transition-transform duration-500 group-hover/avatar:scale-105">
                                    ...
                                    <AvatarFallback className="text-4xl font-black text-gray-200 bg-gray-50">{profile.full_name[0]?.toUpperCase()}</AvatarFallback>
                                </Avatar>
                                {profile.is_verified_expert && (
                                    <div className="absolute -bottom-1 -right-1 bg-white p-1.5 rounded-2xl shadow-lg border border-gray-50">
                                        <div className="bg-[#0c9789] p-1.5 rounded-xl">
                    ...
                    <div className="pt-20 pb-10 px-10">
                        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
                            <div className="space-y-4">
                                <div>
                                    <div className="flex items-center gap-3 mb-1">
                                        <h1 className="text-4xl font-black text-gray-800 tracking-tight">{profile.full_name}</h1>
                                        {profile.is_verified_expert && (
                                            <span className="bg-[#f0fdfa] text-[#0c9789] text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider border border-[#0c9789]/20">
                                                Doğrulanmış
                                            </span>
                                        )}
                                    </div>
                                    {profile.username && (
                                        <p className="text-[#0c9789] font-bold text-lg">@{profile.username}</p>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-wrap items-center gap-4">
                                    <div className={cn(
                                        ...
                                        profile.role === 'teacher' ? "bg-[#f0fdfa] text-[#0c9789] border-[#0c9789]/20" :
                                            profile.role === 'student' ? "bg-[#f0fdfa] text-[#0c9789] border-[#0c9789]/20" :
                                    ...
                                    {profile.custom_id && (
                                        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 border border-gray-100 rounded-2xl text-gray-400 font-mono text-xs font-bold">
                                            <span className="opacity-50 text-base">#</span>
                                            {profile.custom_id}
                                        </div>
                                    )}

                                    <div className="flex items-center gap-2 text-gray-400 font-bold text-xs uppercase tracking-widest bg-gray-50/50 px-3 py-1.5 rounded-2xl border border-gray-100/50">
                                        <Clock className="h-3.5 w-3.5 text-[#0c9789]" />
                            ...
                            <div className="flex items-center gap-3">
                                    ...
                                    <>
                                        {connectionStatus === 'accepted' ? (
                                            <Button onClick={handleMessage}
                                                className="bg-gray-900 hover:bg-gray-800 text-white rounded-[20px] px-8 h-12 font-bold shadow-xl shadow-gray-200 transition-all hover:-translate-y-1">
                                                <MessageCircle className="h-5 w-5 mr-2" /> Mesaj Gönder
                                            </Button>
                                        ) : connectionStatus === 'none' ? (
                                            <Button onClick={handleConnect} disabled={actionLoading}
                                                className="bg-[#0c9789] hover:bg-[#0a7c70] text-white rounded-[20px] px-8 h-12 font-bold shadow-xl shadow-[#0c9789]/20 transition-all hover:-translate-y-1">
                                                {actionLoading ? "..." : "Bağlantı Kur"}
                                            </Button>
                                        ) : connectionStatus === 'pending_sent' ? (
                                            <div className="flex items-center gap-3 px-6 py-3 bg-gray-50 border border-gray-100 rounded-[20px] text-gray-500 font-bold text-sm">
                                ...
                                {connectionStatus === 'self' && (
                                    <Link href="/profile">
                                        <Button variant="outline" className="rounded-[20px] px-8 h-12 font-bold border-gray-200 hover:bg-gray-50 transition-all hover:-translate-y-1">
                                            Profil Düzenle
                                        </Button>
                                    </Link>
                                )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="md:col-span-2 space-y-6">
                            {(profile.bio || profile.special_note) ? (
                                <>
                                    {profile.bio && (
                                        <div className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-sm transition-all hover:shadow-md">
                                            <div className="flex items-center gap-3 mb-6">
                                                <div className="w-1.5 h-6 bg-[#0c9789] rounded-full" />
                                                <h2 className="font-black text-sm uppercase tracking-[0.2em] text-gray-400">Hakkında</h2>
                                            </div>
                                            <p className="text-gray-700 leading-[1.8] text-lg font-medium whitespace-pre-wrap">{profile.bio}</p>
                                        </div>
                                    )}
                                    ...
                                    {profile.special_note && (
                                        <div className="bg-[#fcece6]/30 rounded-[32px] p-8 border border-[#e89b7b]/10 shadow-sm transition-all hover:shadow-md">
                                            ...
                                            <div className="bg-white/60 backdrop-blur-sm p-6 rounded-2xl border border-white/40">
                                                <p className="text-gray-700 leading-relaxed font-bold italic">{profile.special_note}</p>
                                            </div>
                                        </div>
                                    )}
                                </>
                                ...
                                <div className="text-center py-20 bg-gray-50 rounded-[40px] border border-dashed border-gray-200">
                                    ...
                                    <p className="text-gray-400 font-bold">Bu kullanıcı henüz bio eklememiş.</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            {profile.social_links && Object.values(profile.social_links).some(v => v) && (
                                <div className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-sm">
                                    <h2 className="font-black text-xs uppercase tracking-widest text-gray-400 mb-6">Sosyal Medya</h2>
                                    <div className="flex flex-col gap-3">
                                        {profile.social_links.instagram && (
                                            <a href={profile.social_links.instagram.startsWith('http') ? profile.social_links.instagram : `https://instagram.com/${profile.social_links.instagram}`}
                                                ...
                                                className="flex items-center justify-between p-4 bg-[#f0fdfa] text-[#0c9789] rounded-2xl font-bold transition-all hover:-translate-x-1">
                                                <div className="flex items-center gap-3">
                                        ...
                                        {profile.social_links.twitter && (
                                            <a href={profile.social_links.twitter.startsWith('http') ? profile.social_links.twitter : `https://twitter.com/${profile.social_links.twitter}`}
                                                target="_blank" rel="noopener noreferrer"
                                                className="flex items-center justify-between p-4 bg-gray-900 text-white rounded-2xl font-bold transition-all hover:-translate-x-1">
                                                <div className="flex items-center gap-3">
                            ...
                            {/* Trust Card */}
                            <div className="bg-gradient-to-br from-[#0c9789] to-[#0a7c70] rounded-[32px] p-8 text-white shadow-xl shadow-[#0c9789]/20 relative overflow-hidden group">
